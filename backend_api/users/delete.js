    ...
    if (req.query["username"]) {
        permission(req.query["session"], 1)
            .then(() => {
                //* 获取User的id
                var userId;
                db.query("SELECT id FROM users WHERE username=" + db.escape(req.query["username"]) + ";",
                    (err, data) => {
                        if (err) {
                            console.log(err);
                            return error(err);
                            ...
                            if (data != []) {
                                userId = data[0].id;
                                db.query("DELETE FROM sessions WHERE user=" + db.escape(userId) + ";",
                                    (err, data) => {
                                        if (err) {
                                            console.log(err);
                                            return error(err);
                                        } else {
                                            db.query("DELETE FROM users WHERE id=" + db.escape(userId) + ";",
                                                (err, data) => {
                                                    if (err) {
                                                        console.log(err);
                                                        return error(err);
